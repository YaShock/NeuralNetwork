...
public:
    ...
    explicit Network(const std::vector<size_t>& layers,
    ...
    // Returns final score (from validate)
    double train(const Matrix<DataType>& samples,
                 ...
                 bool shuffle = true);
    double validate(const Matrix<DataType>& testSamples, const Matrix<DataType>& testLabels);
...
private:
    ...
    void backpropagation(const ColVec<DataType>& sample,
    ...
    ColVec<DataType> costDerivative(const ColVec<DataType>& output,
                                    const ColVec<DataType>& expected);
